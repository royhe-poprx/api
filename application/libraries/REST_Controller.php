...
abstract class REST_Controller extends CI_Controller {
     ...
     * Fields: db, ignore_limits, key, level, UserID
     * Note: This is a dynamic object (stdClass)
     *
     * @var object
     */
     ...
     * @var bool
     */
    protected $_decryption_success = 0;
     ...
     * @var bool
     */
    protected $_encrypted_request = TRUE;
    ...
    /**
     * Information about the current session user
     *
     * @var object
     */
    ...
    public function __construct($config = 'rest') {
        ...
        header("Access-Control-Allow-Headers: api-key, login-session-key, Origin, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method");
        ...
        if ($this->auth_override !== TRUE) {
            ...
            $this->_detect_login_session_key();
        }
    ...
    public function _remap($object_called, $arguments) {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE) {
            ...
            $this->response([
                ...
                "StatusCode" => self::HTTP_FORBIDDEN,
                "ServiceName" => "",
                "Message" => "Invalid API Key" . $this->rest->key,
                "Errors" => [],
                "Data" => [],
                "ElapsedTime" => "",
            ]);
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE) {
            ...
            if ($this->config->item('rest_enable_limits') && $this->_check_limit($controller_method) === FALSE) {
                $this->response([
                    "Status" => FALSE,
                    "StatusCode" => 401,
                    "Message" => "This API key has reached the time limit for this method.",
                        ], self::HTTP_UNAUTHORIZED);
            }
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE) {
        ...
        if ($this->config->item('rest_enable_logging') === TRUE) {
            ...
            $this->_log_response_data($output);
        }
    ...
    protected function _detect_api_key() {
        ...
        if (($KeyGUID = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            $IsActive = safe_array_key($Row, 'IsActive', NULL);
            $UnderMaintenance = safe_array_key($Row, 'UnderMaintenance', NULL);
            $KeyGUID = safe_array_key($Row, 'KeyGUID', NULL);
            $Level = safe_array_key($Row, 'Level', NULL);
            ...
            } elseif ($UnderMaintenance == 1) {
                $this->response([
                    "Status" => FALSE,
                    "StatusCode" => 503,
                    "Message" => "Sever is down for maintenance",
                ]);
            } else {


                $this->rest->key = $KeyGUID;
            ...
            $this->response([
                "Status" => FALSE,
                "StatusCode" => 401,
                "Message" => "API key is required to consume resources.",
            ]);
        }
    }
     ...
     * @return bool
     */
    protected function _detect_login_session_key() {
        // Get the api key name variable set in the rest config file
        $login_session_key_variable = 'login_session_key';

        // Work out the name of the SERVER entry based on config
        $log_session_key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $login_session_key_variable));

        $this->rest->LoginSessionKey = NULL;
        $this->rest->UserID = NULL;
        ...
        // Find the key from server or arguments
        if (($LoginSessionKey = isset($this->_args[$login_session_key_variable]) ? $this->_args[$login_session_key_variable] : $this->input->server($log_session_key_name))) {

            $Row = $this->rest->db
                            ->where('LoginSessionKey', $LoginSessionKey)
                            ->get('ActiveLogins')->row_array();


            

            $UserID = safe_array_key($Row, 'UserID', NULL);
            $ActiveUserID = safe_array_key($Row, 'ActiveUserID', NULL);
            $IsBlocked = safe_array_key($Row, 'IsBlocked', NULL);
            if (is_null($UserID)) {
                ...
                // Session has been blocked by admin
                $this->response([
                    "Status" => FALSE,
                    "StatusCode" => 401,
                ...
                $this->rest->LoginSessionKey = $LoginSessionKey;
                $this->rest->UserID = $UserID;
                $this->rest->ActiveUserID = $ActiveUserID;
            }
        } else {
            // Session key is not valid
            $this->response([
                "Status" => TRUE,
                "StatusCode" => 401,
                "Message" => "Login Session key is required.",
            ]);
        }
    }
    ...
    function _decrypt_body() {
        $tmp = "";
        $TokenGUID = $this->input->server('HTTP_TOKEN_GUID');
        if (isset($TokenGUID)) {
            $Token = $this->rest->db
                    ...
                    ->row_array();
            if (!is_null($Token)) {
                $tmp = $this->input->raw_input_stream = decrypt_raw_to_json($this->input->raw_input_stream, $Token['EncryptionKey'], $Token['EncryptionIV']);
                $this->rest->db->update('Tokens', array(
                    'LastUsed' => DATETIME
                        ), array(
                    'TokenGUID' => $TokenGUID
                ));
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                $this->_decryption_success = TRUE;
            } else {
                $this->_decryption_success = json_last_error();
            }
        } else {
            $this->_encrypted_request = FALSE;
        }
    }

    function _encrypt_body($Output) {
        if (is_null($Output)) {
            return NULL;
        }
        $TokenGUID = $this->input->server('HTTP_TOKEN_GUID');
        if (isset($TokenGUID)) {
            $Token = $this->rest->db
                    ...
                    ->row_array();
            if (!is_null($Token)) {
                $Output = encrypt_json_to_row($Output, $Token['EncryptionKey'], $Token['EncryptionIV']);
            }
        }
        return $Output;
    }
